...
3. **Review the Changes**: Use agents to analyze the code changes for:
   - CLAUDE.md/AGENTS.md compliance (only applicable instructions)
   ...
   - Unwanted log statements or debug code.
   - Security vulnerabilities or sensitive information exposure
   - Performance issues or anti-patterns
   - Code duplications or maintainability concerns
...
5. **Post Review**: If high-confidence issues found:
   ...
   az devops invoke --area git --resource pullRequestThreads \
     ...
     --in-file /tmp/pr_review_body.json \
     --org {orgUrl}
   ```

   Write the JSON body to `/tmp/pr_review_body.json` first (using `cat > /tmp/pr_review_body.json << 'ENDJSON' ... ENDJSON`), then pass it via `--in-file`. The `--body` flag is not supported by `az devops invoke`.
